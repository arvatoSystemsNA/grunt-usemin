...
//
const defaultBlockReplacements = {
  css: function (block) {
    const media = block.media ? ' media="' + block.media + '"' : '';
    return '<link rel="stylesheet" href="' + block.dest + '"' + media + '>';
  },
  js: function (block) {
    const defer = block.defer ? 'defer ' : '';
    const async = block.async ? 'async ' : '';
    return '<script ' + defer + async + 'src="' + block.dest + '"><\/script>';
  }
};

const FileProcessor = module.exports = function (type, patterns, finder, logcb, blockReplacements) {
  if (!type) {
...
FileProcessor.prototype.replaceBlocks = function replaceBlocks (file) {
  ...
  file.blocks.forEach(function (block) {
    const blockLine = block.raw.join(linefeed);
...
FileProcessor.prototype.replaceWith = function replaceWith (block) {
  let result;
  const conditionalStart = block.conditionalStart ? block.conditionalStart + '\n' + block.indent : '';
  const conditionalEnd = block.conditionalEnd ? '\n' + block.indent + block.conditionalEnd : '';
...
FileProcessor.prototype.replaceWithRevved = function replaceWithRevved (lines, assetSearchPath) {
  ...
  regexps.forEach(function (rxl) {
    ...
    content = content.replace(rxl[0], function (match, src) {
      ...
      const res = match.replace(src, filterOut(file));
      if (srcFile !== file) {
...
FileProcessor.prototype.process = function (filename, assetSearchPath) {
  ...
  const content = this.replaceWithRevved(this.replaceBlocks(this.file), this.file.searchPath);
