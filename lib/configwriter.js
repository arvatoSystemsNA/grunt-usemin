...
const deepMerge = function (origCfg, cfg) {
  const outCfg = origCfg;
  ...
  if (origCfg.files && cfg.files) {
    let done = false;
    for (let i = 0; i < origCfg.files.length; i++) {
...
// For example:
//
//   const w = new ConfigWriter( ['concat', 'uglify'], [], {input: 'app', staging: '.tmp', dest: 'dist'});
//
//   We'll:
...
//
//
const ConfigWriter = module.exports = function (flow, dirs) {
  const self = this;
  this.flow = flow;
...
ConfigWriter.prototype.forEachStep = function (blockType, cb) {
  const writersCount = this.stepWriters(blockType).length;
...
ConfigWriter.prototype.process = function (file, config) {
  ...
  lfile.blocks.forEach(function (block) {
    ...
    self.forEachStep(block.type, function (writer, last) {
      ...
      const blockConfig = writer.createConfig(context, block);
