...
//
const getBlocks = function (content) {
  ...
  //   * 3 : the output
  //
  const regbuild = /<!--\s*build:(\w+)(?:\(([^\)]+)\))?\s*([^\s]+)\s*-->/;
  ...
  lines.forEach(function (l) {
    ...
    const build = l.match(regbuild);
    const endbuild = regend.test(l);
    const startFromRoot = false;
    ...
    const isConditionalStart = l.match(/(<!--\[if.*\]>)(<!-->)?( -->)?/g);
    const isConditionalEnd = l.match(/(<!--\s?)?(<!\[endif\]-->)/g);
    ...
    if (block && last) {
      const asset = l.match(/(href|src)=["']([^'"]+)["']/);
      if (asset && asset[2]) {
        last.src.push(asset[2]);

        const media = l.match(/media=['"]([^'"]+)['"]/);
